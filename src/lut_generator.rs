...
impl LutGenerator<u32> {
    #[inline]
    pub const fn generate_lut(poly: u32) -> [u32; 256] {
        let generator = poly;
        let mut table = [0; 256];

        c_for!(let mut dividend = 0; dividend < 256; dividend += 1; {
            let mut currbyte = (dividend << 24) as u32;
...
mod tests {
    ...
    #[test]
    fn test_crc32_table() {
        const TABLE: [u32; 256] = LutGenerator::<u32>::generate_lut(0x04C11DB7);
        let x = TABLE[0];
        assert_eq!(0x0000, x);

        let x = TABLE[1];
        assert_eq!(0x04C11DB7, x);

        let x = TABLE[255];
        assert_eq!(0xB1F740B4, x);
    }
}
